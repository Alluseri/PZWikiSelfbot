...
function buildHeaders(myHeaders, doc) {
	...
	myHeaders["user-agent"] = "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.0.4692.99 Safari/537.36 OPR/83.0.4254.70";
	if (doc) {
	...
	myHeaders["dnt"] = "1";
	myHeaders["sec-ch-ua"] = '"Chromium";v="96", "Opera GX";v="83", ";Not A Brand";v="99"';
...
async function getEditData(article) {
	var text = await (await fetch("https://pzwiki.net/wiki/"+article, {
		method: "GET",
		credentials: "same-origin",
		headers: buildHeaders({},true)
	})).text();
	console.log("[STEP/SUCCESS] Pretended to open the wiki article, data will be parsed.");
	...
	return {
		...
		"revisionID": text.match('"wgRevisionId":(.+?),')[1],
		"editingStatsID": editx.match('id="editingStatsId" value="(.+?)"')[1],
		"body": editx.match(/(?<=<textarea .+? id="wpTextbox1" .+?>).+?(?=<\/textarea>)/gs)[0]
	};
}
...
async function stashEdit(article, body, summary, csr) {
	var resp = await (await fetch("https://pzwiki.net/w/api.php", {
		...
		body: Object.entries({
			...
			baserevid: csr["revisionID"],
			text: body,
			token: PZWIKI_TOKEN
		}).map((r)=>encodeURIComponent(r[0])+"="+encodeURIComponent(r[1])).join("&"),
		headers: buildHeaders({
...
async function publish(article, body, summary, csr, captcha) {
	var num = Number.parseInt(csr["revisionID"]);
	
	var formData = new FormData();
	formData.append("wpUnicodeCheck", "\u2133\ud835\udcb2\u2665\ud835\udcca\ud835\udcc3\ud835\udcbe\ud835\udcb8\u2134\ud835\udcb9\u212f");
	formData.append("wpAntispam", "");
	formData.append("editingStatsId", csr["editingStatsId"]);
	captcha && formData.append("g-recaptcha-response", captcha);
	formData.append("wpSection", "");
	formData.append("wpStarttime", csr["date"]);
	...
	return (await (await fetch("https://pzwiki.net/w/index.php?title="+article+"&action=submit", {
		method: "POST",
		credentials: "same-origin",
		body: formData,
		headers: buildHeaders({
...
async function fullEditCycle(article, body, summary, timeTaken) {
	try {
		...
		setTimeout(async()=>{
			...
			if (!PZWIKISELFBOT_DISABLE_CAPTCHA && await publish(article, body, summary, csr)) {
				console.log("Check the page.");
				renderCaptcha(article, body, summary, csr);
			} else console.log("фрешхак забустил");
		},timeTaken);
	} catch (ex) {
...
async function fullEditCycleCallback(article, callback, summary, timeTaken) {
	try {
		console.log("0/3 Pushing edit state");
		var csr = await getEditData(article);
		console.log("1/3 Editing existing content and taking time");
		var body = callback(csr["body"]);
		setTimeout(async()=>{
			console.log("2/3 Dropping stash edit");
			await stashEdit(article, body, summary, csr);
			console.log("3/3 Publishing");
...
async function renderCaptcha(article, body, summary, csr) {
	document.body.innerHTML = '<span style="font-family: Roboto, Arial;"> You tried editing a page with introducing new external links, OR you got suspected in being a bot. Just... complete the captcha.<br>- Nekoseri</span><br>';
	var dv = document.createElement('div');
	dv.id = "nekoseri_captcha";
	dv.style = "font-family: Roboto, Arial;";
	dv.innerText = "Please wait, loading reCAPTCHA...";
	document.body.appendChild(dv);
	window["bynekoseri_captcha"] = function() {
		console.log("Loaded recaptcha framework.");
		dv.innerText = "Loaded reCAPTCHA, rendering...";
		grecaptcha.render('nekoseri_captcha', {
			'callback': async (x) => {
				try {
					if (await publish(article, body, summary, csr, x)) {
						dv.style = "font-family: Roboto, Arial;color: red;";
						dv.innerText = "You didn't pass the reCAPTCHA check. Please try again.";
					} else {
						dv.style = "font-family: Roboto, Arial;color: green;";
						dv.innerText = "Congratulations, your edit has been published.";
					}
					console.log("фрешхак забустил");
				} catch (ex) {
					console.log("фрешхак не забустил");
					console.log(ex);
					dv.style = "font-family: Roboto, Arial;color: darkred;";
					dv.innerText = "There was an error while publishing, check the console...";
				}
			},
			'sitekey': '6LeBTBcTAAAAAETBveoI74AzxhPZz2-4I5JA0p0B'
		});
	}
	var scr = document.createElement("script");
	scr.src = "https://www.google.com/recaptcha/api.js?onload=bynekoseri_captcha&render=explicit&hl=en";
	document.body.appendChild(scr);
}

var PZWIKI_TOKEN = mw.user.tokens.values["editToken"];
var PZWIKI_COOKIE = document.cookie;
var PZWIKISELFBOT_DISABLE_CAPTCHA = false;
